...
def _robust_strip_fences(raw: str) -> str:
    """Removes ```json ... ``` blocks even with leading whitespace or weird formatting."""
    if not raw: return ""
    # Remove markdown code fences
    cleaned = re.sub(r"^[\s\n]*```(?:json)?", "", raw, flags=re.MULTILINE | re.IGNORECASE)
    cleaned = re.sub(r"```[\s\n]*$", "", cleaned, flags=re.MULTILINE | re.IGNORECASE)
    return cleaned.strip()


def _repair_json(raw: str) -> str:
    """Very basic cleanup for common LLM JSON mishaps (trailing commas, etc)."""
    if not raw: return ""
...
async def run_academic_pipeline(
    ...
    """
    global pipeline_running, _pipeline_running_ref, academic_results, stream_frame_counters, _shared_llm
    ...
    try:
        ...
        # ── v3.2.2: Release Whisper VRAM before Phi-3 loads ───────────────
        # On RTX 3050, faster-whisper holds ~300MB VRAM. Phi-3 4-bit needs
        # ~2.5GB + peak overhead during quantisation. Without this release,
        # the process is silently killed by Windows OOM.
        try:
            release_whisper_models()
            logger.info("[Memory] Whisper VRAM released — Phi-3 can now load safely.")
        except Exception as exc:
            logger.warning(f"[Memory] Whisper release failed (non-fatal): {exc}")
...
async def delete_lecture(
    ...
    # 4. Remove from in-memory results
    targets = [k for k, v in academic_results.items() if video_stem in k]
    for tk in targets:
