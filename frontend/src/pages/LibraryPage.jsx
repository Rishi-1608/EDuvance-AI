...
import { Book, Play, Clock, ChevronRight, FileText, Layers, HelpCircle, Search, Filter, Trash2, Image as ImageIcon, Mic, Radio } from 'lucide-react';
...
export function LibraryPage({ onSelectVideo }) {
  const getMediaConfig = (type) => {
    switch (type) {
      case 'video':
        return { icon: <Play size={20} />, btnIcon: <Play size={10} />, label: 'Video', btnText: 'Play Video', color: 'video', activityColor: 'var(--nebula-blue)' };
      case 'images':
        return { icon: <ImageIcon size={20} />, btnIcon: <ImageIcon size={10} />, label: 'Images', btnText: 'View Photos', color: 'images', activityColor: 'var(--nebula-rose)' };
      case 'audio':
        return { icon: <Mic size={20} />, btnIcon: <Mic size={10} />, label: 'Audio', btnText: 'Listen Audio', color: 'audio', activityColor: 'var(--nebula-violet)' };
      case 'document':
      case 'pdf':
        return { icon: <FileText size={20} />, btnIcon: <FileText size={10} />, label: 'Document', btnText: 'Read Doc', color: 'document', activityColor: 'var(--nebula-teal)' };
      case 'live':
        return { icon: <Radio size={20} />, btnIcon: <Radio size={10} />, label: 'Live Lecture', btnText: 'View Session', color: 'live', activityColor: 'var(--success)' };
      default:
        return { icon: <Play size={20} />, btnIcon: <Play size={10} />, label: 'Media', btnText: 'View', color: 'video', activityColor: 'var(--accent-primary)' };
    }
  };
              ...
              >
                <div className="library-card-header">
                  <div
                    ...
                    style={{ '--widget-color': getMediaConfig(video.input_type).activityColor }}
                  >
                    <span className="stroke-glow">
