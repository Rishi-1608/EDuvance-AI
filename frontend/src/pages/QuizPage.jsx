...
import { getQuiz, submitQuizSession } from '@/lib/api';
...
export function QuizPage({ preselectedStem }) {
  ...
  const finishQuiz = async () => {
    ...
    try {
      if (selectedStem) {
        await submitQuizSession(selectedStem, validQs.length, score);
      }
      setDone(true);
    } catch (e) {
      console.error("Failed to save quiz session:", e);
      // Still show results even if saving fails, but maybe log it
      setDone(true);
    } finally {
      setSaving(false);
    }
  ...
  const renderBody = () => {
    if (done) {
      return (
        <div className="quiz-content-wrapper flex-1 overflow-y-auto px-10 py-8 flex flex-col items-center justify-center relative z-0">
          <ScoreBoard score={score} total={validQs.length} onRetry={retry} />
        </div>
      );
    }
    ...
    return (
      <div className="quiz-content-wrapper flex-1 overflow-y-auto px-10 py-8 flex flex-col relative z-0">
        <div className="questions-list space-y-6 pb-32">
          {validQs.map((q, i) => (
            <QuizQuestion
              key={i}
              question={q}
  ...
  return (
    <div className="quiz-layout-container animate-in fade-in zoom-in-95 duration-300">
      ...
      <div className="quiz-main flex-1 overflow-hidden flex flex-col relative z-0">
        ...
        {/* Floating Bottom Action Bar - Outside scrollable area for fixed positioning */}
        {allAnswered && !done && (
          <div className="quiz-completion-bar animate-in slide-in-from-bottom duration-500">
            <div className="completion-content glass">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-success/20 flex items-center justify-center text-success">
                  <CheckCircle2 size={20} />
                </div>
                <div>
                  <h4 className="text-white font-semibold m-0">Quiz Complete</h4>
                  <p className="text-muted text-xs m-0">Review your answers or see final score</p>
                </div>
              </div>
              <button 
                ...
                disabled={saving}
              >
                {saving ? <span className="spinner-sm mr-2" /> : <Trophy size={18} className="mr-2" />}
